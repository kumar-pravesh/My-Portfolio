...
import { Search, Code2, Sparkles, X, CheckCircle2, Layers } from "lucide-react";
...
const ProjectsPage = () => {
  ...
  useEffect(() => {
    document.title = "Projects — Pravesh Kumar Portfolio";
    let isMounted = true;
    publicApi
      .getProjects()
      .then((data) => {
  ...
  return (
    <div className="pt-20 sm:pt-24 pb-20 px-[5%] lg:px-[9%] min-h-screen max-w-7xl mx-auto space-y-8 sm:space-y-10">
      ...
      <div className="relative text-center max-w-3xl mx-auto space-y-4 pt-4">
        ...
        >
          <Code2 size={12} /> Production Engineering & Software Showcase
        </motion.div>
        ...
        >
          Explore production systems, web applications, microservices, and
          enterprise tools engineered by Pravesh Kumar.
        </motion.p>
        ...
        >
          ...
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder="Search by project name, tech stack, or Ref ID..."
              value={search}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <div
              key={n}
              className="bg-[#0b1528] rounded-2xl border border-white/5 animate-pulse h-80"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 bg-[#0b1528]/80 rounded-2xl border border-white/10 max-w-md mx-auto space-y-3 text-slate-400">
          ...
          <p className="text-xs text-slate-400">
            Try adjusting your search query or active category.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((project, index) => (
            <motion.div
              key={project.reference_id || index}
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
